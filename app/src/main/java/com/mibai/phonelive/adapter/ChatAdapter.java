...
import com.mibai.phonelive.utils.MessageDateUtil;
...
public class ChatAdapter extends RecyclerView.Adapter {
    ...
    class Vh extends RecyclerView.ViewHolder {
        ...
        void setData(ChatMessageBean bean, int position, Object payload) {
            if (payload == null) {
                if (bean.isFromSelf()) {
                    String imgUrl = mUserAvatar;
                    if (mUserAvatar.startsWith("http") || mUserAvatar.startsWith("https")) {
                        ImgLoader.displayAvatar(imgUrl, mAvatar);
                    } else {
                        ImgLoader.displayAvatar(AppConfig.HOST + imgUrl, mAvatar);
                    }
                } else {
                    ImgLoader.displayAvatar(mToUserAvatar, mAvatar);
                }
                if (position == 0) {
