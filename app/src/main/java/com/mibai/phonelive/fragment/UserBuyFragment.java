...
import com.mibai.phonelive.adapter.UserBuyAdapter;
...
public class UserBuyFragment extends UserItemFragment implements OnItemClickListener<VideoBean> {
    ...
    protected void main() {
        ...
        mRefreshView.setDataHelper(new RefreshView.DataHelper<VideoBean>() {
            ...
            public RefreshAdapter<VideoBean> getAdapter() {
                if (mAdapter == null) {
                    mAdapter = new UserBuyAdapter(mContext);
                    mAdapter.setOnItemClickListener(UserBuyFragment.this);
                }
                return mAdapter;
            }
            ...
            public void loadData(int p, HttpCallback callback) {
                if (AppConfig.getInstance().isLogin()) {
                    HttpUtil.getBuyVideos(mUid, p, callback);
                }
            }
    ...
    public void onDestroy() {
        ...
        HttpUtil.cancel(HttpUtil.GET_BUY_VIDEOS);
        super.onDestroy();
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onNeedRefreshBuyEvent(NeedRefreshBuyEvent e) {
        if (mRefreshView != null) {
            mRefreshView.initData();
        }
    }
