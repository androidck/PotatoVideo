...
import com.mibai.phonelive.activity.EditProfileActivity;
import com.mibai.phonelive.activity.FansActivity;
import com.mibai.phonelive.activity.FollowActivity;
...
import com.mibai.phonelive.activity.SettingActivity;
import com.mibai.phonelive.activity.ShareActivity;
import com.mibai.phonelive.activity.VideoPlayActivity;
import com.mibai.phonelive.activity.WalletActivity;
import com.mibai.phonelive.activity.WebUploadImgActivity;
import com.mibai.phonelive.activity.ZanMoneyActivity;
import com.mibai.phonelive.bean.ConfigBean;
import com.mibai.phonelive.bean.UserBean;
import com.mibai.phonelive.custom.ViewPagerIndicator;
...
import com.mibai.phonelive.interfaces.AppBarStateListener;
...
public class UserFragment extends AbsFragment implements ViewPager.OnPageChangeListener, View.OnClickListener {
    ...
    protected void main() {
        ...
        mAvatar = (RoundedImageView) mRootView.findViewById(R.id.avatar);
        ...
        tv_share_new = (TextView) mRootView.findViewById(R.id.tv_share_new);
        tv_vip_pay = (TextView) mRootView.findViewById(R.id.tv_vip_pay);
        tv_my_qb = (TextView) mRootView.findViewById(R.id.tv_my_qb);
        ly_pay = mRootView.findViewById(R.id.ly_pay);

        tv_login = mRootView.findViewById(R.id.tv_login);
        tv_tips = mRootView.findViewById(R.id.tv_tips);
        tv_share = mRootView.findViewById(R.id.tv_share);
        id_expire = mRootView.findViewById(R.id.id_expire);
        ...
        mBuyString = WordUtil.getString(R.string.buy);
        mFans.setOnClickListener(this);
        mFollow.setOnClickListener(this);
        mBtnMore.setOnClickListener(this);
        tv_share.setOnClickListener(this);
        tv_login.setOnClickListener(this);
        tv_share_new.setOnClickListener(this);
        tv_vip_pay.setOnClickListener(this);
        tv_my_qb.setOnClickListener(this);
        ...
        mIndicator.setTitles(new String[]{mWorkString, mLikeString, mBuyString});
        mViewPager = (ViewPager) mRootView.findViewById(R.id.viewPager);
        mFragmentList = new ArrayList<>();
        mFragmentList.add(new UserWorkFragment());
        mFragmentList.add(new UserLikeFragment());
        mFragmentList.add(new UserBuyFragment());
    ...
    private void showUserInfo() {
        if (mUserBean != null) {
            ...
            if (mIndicator != null) {
                ...
                mIndicator.setTitleText(2, mBuyString + " " + mUserBean.getBuyVideos());
            }
            if (mIsMainUserCenter && mViewPager != null && mViewPager.getCurrentItem() == 0) {
                ((MainFragment) getParentFragment()).showRecordTip(mUserBean.getWorkVideos() == 0);
            }
            if (AppConfig.getInstance().getUid().equals(mUserBean.getId())) {
                id_expire.setVisibility(View.VISIBLE);
                // 是会员
                if ("1".equals(mUserBean.getIsvip())) {
                    long time = Long.parseLong(mUserBean.getVip_endtime());
                    String result = new SimpleDateFormat("yyyy-MM-dd").format(time * 1000);
                    id_expire.setText("到期时间：" + result);
                } else {
                    id_expire.setText("今日观影次数：" + mUserBean.getFront_freetimes() + "/" + mUserBean.getFree_times());
                }
            } else {
                id_expire.setVisibility(View.GONE);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.tv_vip_pay:
                startActivity(new Intent(mContext, VipPayActivity.class));
                break;
            case R.id.tv_my_qb:
