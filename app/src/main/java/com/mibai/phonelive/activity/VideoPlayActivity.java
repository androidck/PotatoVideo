...
import com.mibai.phonelive.fragment.VideoInputFragment;
import com.mibai.phonelive.fragment.VideoPlayFragment;
import com.mibai.phonelive.fragment.VideoShareFragment;
...
public class VideoPlayActivity extends AudioAbsActivity implements ViewPager.OnPageChangeListener, GlobalLayoutChangedListener, VideoChangeListener {
    ...
    public static void forwardVideoPlay(Context context, String videoKey, int videoPosition, int videoPage, UserBean userBean, int isAttention, int identifying) {
        Intent intent = new Intent(context, VideoPlayActivity.class);
        intent.putExtra(Constants.VIDEO_KEY, videoKey);
        intent.putExtra(Constants.VIDEO_POSITION, videoPosition);
        intent.putExtra(Constants.VIDEO_PAGE, videoPage);
        intent.putExtra(Constants.USER_BEAN, userBean);
        intent.putExtra(Constants.IS_ATTENTION, isAttention);
        intent.putExtra(Constants.IDENTIFYING, identifying);
        context.startActivity(intent);
    }
...
//                }
    ...
    private VideoPlayWrap.ActionListener mActionListener = new VideoPlayWrap.ActionListener() {
        ...
        public void onAdvClick(VideoPlayWrap wrap, VideoBean bean) {
            ...
            Intent intent = new Intent(VideoPlayActivity.this, WebActivity.class);
